...
| 30-3 | Rotate dev DB creds: read from env in `001_roles.sql` via `:var` substitution; add fail-fast in `main.ts` if `NODE_ENV=production` and `DATABASE_URL` contains `dev_password`. | 2 hours | done | PR #31 (Task 21) — `001_roles.sql` uses psql `:var` substitution (`:app_role_password`, `:ops_password`); new `001_roles.sh` wrapper for docker-compose init; CI passes `-v` flags; `main.ts` adds `assertNoDevCredentialsInProduction()` fail-fast. **All 8 thirty-day blockers are now DONE.** |
...
| Dev DB credentials committed in plaintext | P0 | done | PR #31 (Task 21, 30-3) — `001_roles.sql` now uses psql `:var` substitution (no literals); `main.ts` adds production fail-fast; docker-compose + CI pass dev values via env/flags |
