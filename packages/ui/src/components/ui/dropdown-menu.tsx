...
function DropdownMenuCheckboxItem({
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      // Radix's DropdownMenuCheckboxItemProps declares `checked` as
      // `CheckedState | undefined` (optional), but the underlying
      // CheckboxItem primitive's `checked` prop is `CheckedState` (required).
      // With `exactOptionalPropertyTypes: true` (per
      // @clinic-saas/tsconfig/base), passing `checked={checked}` directly
      // fails because `CheckedState | undefined` is not assignable to
      // `CheckedState`. Conditionally spread `checked` only when defined so
      // the prop is omitted entirely when `undefined` (which is the correct
      // semantic — Radix treats a missing `checked` as "unchecked").
